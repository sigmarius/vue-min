<template>
  <div>
  <div class="mb-2">
    <div>isAuth: {{$store.state.isAuth ? 'User is auth' : 'User is guest'}}</div>
    <div>likes: {{ $store.state.likes }}</div>
    <div>double likes: {{ $store.getters.doubleLikes }}</div>
    <div>
        <ui-button @click="$store.commit('incrementLikes')">Like</ui-button>
        <ui-button @click="$store.commit('decrementLikes')">Dislike</ui-button>
    </div>
  </div>
